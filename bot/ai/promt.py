...
OBSERVATION_INTERPRETATION_PROMT = {
    ...
    "request": "Очень внимательно проанализируй все результаты анализов из документов, которые были отправлены порциями и согласно последним научным данным из интернета о нормах по показателям в документах верни информацию о каждом из анализов на основе 'response_format' в JSON. Не добавляй анализы, которых нет в документе и верни только результаты из документов с отклонениями от нормы!",
    "response_format": {
        "_type": "object",
        "_properties": {
            "results": {
